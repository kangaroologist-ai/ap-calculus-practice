...
import {makePortableProgress,encodeProgress,splitIntoQrFrames,qrErrorCorrection,QrCollector,decodeProgress} from '../src/transfer';
test('full-curriculum snapshot survives raster QR encoding and local image decoding',()=>{
 ...
 for(const frame of [...frames].reverse()){
  const qr=QRCode.create(frame,{errorCorrectionLevel:qrErrorCorrection(frame)}),size=qr.modules.size,scale=3,width=(size+8)*scale,pixels=new Uint8ClampedArray(width*width*4);pixels.fill(255);
