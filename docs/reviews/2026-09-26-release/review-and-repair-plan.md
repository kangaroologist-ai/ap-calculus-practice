...
- [x] **C2-6 · Spec K1/D2** — 提交、推送；比对本地 `dist` 与线上 8 文件 SHA-256。若一致则记录“运行时未变、无需重新部署”；若不一致则 `wrangler pages deploy` 后重新比对。等 CI 结果后记录。**结果：** `4e448a7` 已推送，CI [`36205084958`](https://github.com/kangaroologist-ai/ap-calculus-practice/actions/runs/36205084958) 成功。本地重新构建后与线上逐个比对 SHA-256，8 个文件全部一致（HTML 按 Cloudflare 的 308 规范 URL 跟随跳转后比对；不跟随跳转会得到空响应体，因而误判不一致）。运行时未变，**无需重新部署**，线上仍为 `912748f2`。
...
- `4e448a7` — 第二轮复核：新增键盘前后像素断言（Chromium 4/4 通过，反向验证 4/4 失败）；键盘相关 e2e 9 项通过；单测 320/320；CI `36205084958` 成功；线上 8 个文件与构建一致，不重新部署。

## 第二轮完成核对

- K2′ → C2-2（实现）+ C2-3（反向验证：能抓到原回归）+ C2-4（全矩阵通过）+ CI `36205084958`：满足。
- D2 → C2-1（截图入库）+ C2-5/本条（progress log）：满足。
- K1/K3 未改动，C2-4 重跑通过、C2-6 线上一致：仍满足。
- 未处理且未声称完成：R5 的既有 MathLive 工具栏小问题；真机安全区与摄像头仍未验证。
