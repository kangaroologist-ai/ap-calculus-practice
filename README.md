...
快照含完整 FSRS 状态／参数／版本、最近两题的对错／模板编号／128-bit 题目指纹（不含题目公式）、已解锁等级、复习和诊断队列；不含姓名、账号、完整作答历史和当前草稿。换设备从新题开始。本地刷新则可恢复当前题。

代码是自包含数据，不需要服务端短码。DSP2 使用固定版本字段顺序、去重字典与压缩，保留 FSRS 原始数值精度；旧 DSP1 仍可导入。二维码优先使用单张，较大数据改用 QR 字母数字模式；仅超出单码容量的异常大快照分片。所有分片属于同一快照且收齐后才能导入。禁止自动合并两台设备各自继续练习的历史。导入前自动保留本机备份，可用 Restore backup 撤回。代码并非加密，获得代码的人可读取学习状态；不要将其当作正式成绩凭证。
